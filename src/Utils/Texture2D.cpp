...
int Texture2D::loadTexture(unsigned int formatStorageOpenGL, unsigned int formatImage,
                           unsigned int dataTypeImage) {
  int width, height, nrChannels;
  unsigned char* data = stbi_load(pathToTexture, &width, &height, &nrChannels, 0);
  if (data) {
    glBindTexture(GL_TEXTURE_2D, ID);
    glTexImage2D(GL_TEXTURE_2D, 0, formatStorageOpenGL, width, height, 0, formatImage,
                 dataTypeImage, data);
